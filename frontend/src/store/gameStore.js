...
export const CHARACTERS = {
  yuri: {
    ...
    firstDialogue: '아, 죄송해요... 책을 떨어뜨렸네요. 혹시 주워주실 수 있을까요? 저는 유리라고 해요. 이번에 3학년 1반이에요.',
    conversationHint: '💡 유리는 조용하고 차분한 성격이에요. 책이나 공부 이야기를 하면 좋아할 거예요.',
    color: '#ec4899',
    image: '/characters/yuri.png',
    events: [
      { threshold: 50, message: '유리가 과제를 같이 하자고 제안합니다.' },
      { threshold: 75, message: '유리가 도서관에서 시험 공부를 같이 하자고 합니다.' },
  ...
  jiho: {
    ...
    firstDialogue: '어? 너도 혼자야? 나랑 같이 밥 먹을래? 나 지호야! 농구부인데 ㅋㅋ 같은 반 아니야? 처음 보는 것 같은데!',
    conversationHint: '💡 지호는 밝고 활발한 성격이에요. 운동이나 재미있는 이야기를 하면 좋아할 거예요.',
    color: '#3b82f6',
  ...
  seyeon: {
    ...
    firstDialogue: '...뭐야? 방해하지 마. 지금 중요한 부분 읽고 있어. 필요한 거 있으면 빨리 말해.',
    conversationHint: '💡 세연은 차갑고 논리적인 성격이에요. 지적인 대화나 과학 이야기를 하면 관심을 보일 거예요.',
    color: '#8b5cf6',
...
const useGameStore = create((set, get) => ({
  ...
  increaseIntimacy: (characterId, amount) => {
    ...
    character.events.forEach(event => {
      ...
      if (newIntimacy >= event.threshold && 
    ...
    // 유리와 충분히 대화 후 지호 만남
    if (updatedState.storySequenceIndex === 1 && characterId === 'yuri' && updatedState.intimacy.yuri >= 10) {
      setTimeout(() => {
        get().addMessage({
          type: 'system',
    ...
    // 지호와 충분히 대화 후 세연 만남
    else if (updatedState.storySequenceIndex === 2 && characterId === 'jiho' && updatedState.intimacy.jiho >= 10) {
      setTimeout(() => {
        get().addMessage({
          type: 'system',
          content: '지호와 즐거운 점심시간을 보냈습니다. 방과 후 도서관에 가볼까요?'
        });
        setTimeout(() => get().progressStory(), 2000);
      }, 2000);
    } 
    // 세연과 충분히 대화 후 시간 경과
    else if (updatedState.storySequenceIndex === 3 && characterId === 'seyeon' && updatedState.intimacy.seyeon >= 10) {
      setTimeout(() => {
        get().addMessage({
          type: 'system',
          content: '세 명의 친구들과 모두 인사를 나눴습니다. 이제 본격적으로 우정을 쌓아갈 시간입니다.'
        });
        setTimeout(() => get().progressStory(), 2000);
      }, 2000);
    }
    // 전체 친밀도 기준 스토리 진행
      ...
      const totalIntimacy = updatedState.intimacy.yuri + updatedState.intimacy.jiho + updatedState.intimacy.seyeon;
      if (totalIntimacy >= 120) { // 평균 40 이상
        setTimeout(() => {
          get().addMessage({
            type: 'system',
            content: '친구들과의 관계가 깊어졌습니다. 축제 시즌이 다가오고 있습니다.'
          });
          setTimeout(() => get().progressStory(), 2000);
        }, 2000);
      }
      ...
      const totalIntimacy = updatedState.intimacy.yuri + updatedState.intimacy.jiho + updatedState.intimacy.seyeon;
      if (totalIntimacy >= 200) { // 평균 66 이상
        setTimeout(() => {
          get().addMessage({
            type: 'system',
            content: '소중한 추억들이 쌓였습니다. 이제 졸업이 다가오고 있습니다.'
          });
          setTimeout(() => get().progressStory(), 2000);
        }, 2000);
      }
  ...
  // 메시지 추가
  addMessage: (message) => {
    set({ messages: [...get().messages, { ...message, timestamp: Date.now() }] });
  },
  ...
  removeMessage: (timestamp) => {
    ...
    set({ messages: state.messages.filter(msg => msg.timestamp !== timestamp) });
    ...
    if (message && message.requiresClick && state.waitingForClick) {
      ...
      if (message.type === 'narration' && !state.currentCharacter && state.storySequenceIndex === 1) {
        setTimeout(() => {
      ...
      // 캐릭터 만남 후 대사 표시
        ...
        if (character && state.metCharacters.includes(state.currentCharacter) && 
            state.messages.filter(m => m.character === state.currentCharacter).length === 0) {
          setTimeout(() => {
  ...
  advanceStoryPhase: () => {
    ...
    if (currentIndex < phases.length - 1) {
      ...
      // 단계별 내레이션
      const narrations = {
        [STORY_PHASES.DEVELOPMENT]: '한 달이 지났습니다. 학교 생활에도 익숙해지고, 새로운 친구들과의 관계도 조금씩 깊어지고 있습니다.',
        [STORY_PHASES.CLIMAX]: '5월, 학교 축제와 수학여행 시즌입니다. 친구들과의 관계가 더욱 깊어지는 중요한 시기입니다.',
        [STORY_PHASES.ENDING]: '2월, 졸업식이 다가옵니다. 그동안 쌓아온 우정의 결실을 맺을 시간입니다.'
      };
  ...
  // 입력 모드 전환
  toggleInputMode: () => {
    set({ inputMode: get().inputMode === 'text' ? 'voice' : 'text' });
  },
